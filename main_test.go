...
func Test_example(t *testing.T) {
	sizeAV, err := SnrSize(AviserPost{})
	if err != nil {
		t.Fatal(err)
	}
	lines, err := exampleLines("./test/sample.zip")
	if err != nil {
		t.Fatal(err)
	}
	err = p100(sizeAV, lines[0])
	if err != nil {
		t.Error(err)
	}
	for _, eta := range exampleTestArguments() {
		pXXX(t, sizeAV, lines[1:], eta.v, eta.post)
	}
}
...
func postData886(result []byte) {
	...
	copy(result[postIDLength+47:], p.NembelPrecision)
	copy(result[postIDLength+53:], p.Netotbeslbet)
	copy(result[postIDLength+65:], p.NetotbeslbetPrecision)
	copy(result[postIDLength+71:], p.Nenombel)
	copy(result[postIDLength+83:], p.NenombelPrecision)
...
func postPointer886() *Post886 {
	return &Post886{
		...
		NembelPrecision:       "15",
		Netotbeslbet:          "16",
		NetotbeslbetPrecision: "17",
		Nenombel:              "18",
		NenombelPrecision:     "19",
		Neejvarkst:            "a",
		Neantaktier:           "21",
		Netilstbesl:           "22",
		Netilstav:             "23",
		Netilstdat:            "24",
		Netillsbel:            "25",
		NetillsbelPrecision:   "26",
		Neforfdat:             "27",
		Neforfav:              "28",
		Neforfbel:             "29",
		NeforfbelPrecision:    "30",
		Neforfanta:            "31",
		Ngenom7:               "32",
		Ngenom8:               "33",
		Nlagst:                "34",
